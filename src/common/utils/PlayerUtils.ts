...
export class PlayerHypixelUtils {
    ...
    public getPlayerRank: ({player, formatting}: { player: Player; formatting?: boolean }) => ({ colourHex: string; rankFormatted: string; rankClean: string; htmlCode: string; rankColourHex: MinecraftColoursImpl }) = ({player, formatting = false}: { player: Player, formatting?: boolean }) => {
        ...
        if (hypixelPlayer !== null) {
            ...
            } else switch (hypixelPlayer.newPackageRank) {
                case 'MVP_PLUS':
