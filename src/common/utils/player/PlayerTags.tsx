...
const RenderKeathizTags = (props: PlayerTags) => {
    ...
    if (player.sources.keathiz == null && player.loaded) {
        ...
        if (player?.sources?.keathiz?.status == 200 && player?.sources?.keathiz?.data) {
            ...
            if (keathizTags?.player?.queues?.consecutive_queue_checks?.last_30_queues["1_min_requeue"] ?? 0 >= 15 ?? keathizTags?.player?.queues?.consecutive_queue_checks?.last_10_queues["1_min_requeue"] ?? 0 >= 5 ?? keathizTags?.player?.queues?.consecutive_queue_checks?.last_10_queues["2_min_requeue"] ?? 0 >= 6 ?? keathizTags?.player?.queues?.consecutive_queue_checks?.last_10_queues["3_min_requeue"] ?? 0 >= 8) {
