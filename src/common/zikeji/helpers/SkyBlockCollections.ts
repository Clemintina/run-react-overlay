 ...
 * @category Helper
 */
export function getSkyBlockProfileMemberCollections(profile: Pick<NonNullable<Components.Schemas.SkyBlockProfile>, "members">, collections: Components.Schemas.SkyBlockResourcesParentCollections): SkyBlockProfileCollections | false {
    ...
    (acc, member) => {
      ...
      if (member.collection) {
        Object.entries(member.collection).forEach(([key, quantity]) => {
          ...
          acc.collectionQuantities[key] += quantity /* istanbul ignore next */ || 0;
        });
      }
      return acc;
    },
    ...
    Object.entries(collectionGroup.items).forEach(([collectionId, collection]) => {
      ...
      const child: SkyBlockProfileCollection = {
        ...
        tier: collection.tiers.reduce((tier, tierInfo) => {
          if (profileCollectionValues.unlockedCollTiers.has(`${collectionId}_${tierInfo.tier}`) || (profileCollectionValues.collectionQuantities[collectionId] ?? 0) > (tierInfo.amountRequired /* istanbul ignore next */ || 0)) {
      ...
      if (child.tier === child.maxTier) {
        ...
        child.nextTierAmountRequired =
          ...
          collection.tiers.find((tInfo) => tInfo.tier === child.nextTier)?.amountRequired /* istanbul ignore next */ || 0;
        child.progress = ((profileCollectionValues.collectionQuantities[collectionId] || 0) / child.nextTierAmountRequired) * 100;
      }
