...
enum NETWORK_LEVEL_CONSTANTS {
  START = 10000,
  GROWTH = 2500,
  RPQP = (() => -(NETWORK_LEVEL_CONSTANTS.START - 0.5 * NETWORK_LEVEL_CONSTANTS.GROWTH) / NETWORK_LEVEL_CONSTANTS.GROWTH)(),
...
export function getExpFromNetworkLevel(level: number): number {
  ...
  const expToFlooredLevel = (NETWORK_LEVEL_CONSTANTS.GROWTH * 0.5 * (flooredLevel - 2) + NETWORK_LEVEL_CONSTANTS.START) * (flooredLevel - 1);
...
export function getNetworkLevel(data: Components.Schemas.Player | number): NetworkLevel {
  ...
  const level = Math.floor(1 + NETWORK_LEVEL_CONSTANTS.RPQP + Math.sqrt(NETWORK_LEVEL_CONSTANTS.IPQP + (2 / NETWORK_LEVEL_CONSTANTS.GROWTH) * currentExp));
