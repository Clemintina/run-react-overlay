...
export function totalExpToSkyWarsLevel(level: number): number {
    ...
    return level <= skyWarsLevelConstants.easyLevelExp.length ? acc : acc + (level - skyWarsLevelConstants.easyLevelExp.length) * skyWarsLevelConstants.expPerLevel;
}
 ...
 * @category Helper
 */
export function getSkyWarsLevelInfo(data: Components.Schemas.Player | number): SkyWarsLevelInfo;
export function getSkyWarsLevelInfo(data: Components.Schemas.Player | number, includePrestige: true): SkyWarsLevelInfoAndPrestige;
export function getSkyWarsLevelInfo(data: Components.Schemas.Player | number, includePrestige: false): SkyWarsLevelInfo;
export function getSkyWarsLevelInfo(data: Components.Schemas.Player | number, includePrestige?: boolean): SkyWarsLevelInfoAndPrestige | SkyWarsLevelInfo {
    ...
    if (level === -1) {
        level = skyWarsLevelConstants.easyLevelExp.length + Math.floor((currentExp - expAcc) / skyWarsLevelConstants.expPerLevel);
        expAcc += (level - skyWarsLevelConstants.easyLevelExp.length) * skyWarsLevelConstants.expPerLevel;
    }
    const expToLevel = expAcc;
    const nextLevelExp = expToLevel + (level < skyWarsLevelConstants.easyLevelExp.length ? skyWarsLevelConstants.easyLevelExp[level] : skyWarsLevelConstants.expPerLevel);
