...
export class Client {
    ...
    public constructor(key: string, options?: ClientOptions) {
        ...
        this.userAgent = `Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) ${options?.userAgent ?? ""} AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.164 Safari/537.36`;
        this.cache = options?.cache;
    }

    public on<E extends keyof ClientEvents>(event: E, listener: ClientEvents[E]): this {
        this.emitter.on(event, listener);
        return this;
    }

    public once<E extends keyof ClientEvents>(event: E, listener: ClientEvents[E]): this {
        this.emitter.once(event, listener);
        return this;
    }

    public off<E extends keyof ClientEvents>(event: E, listener: ClientEvents[E]): this {
    ...
    private createActionableCall<T extends Components.Schemas.ApiSuccess>(path: string, parameters: Parameters = {}): ActionableCall<T> {
        ...
        if (path.startsWith("resources") || path === "skyblock/auctions" || path === "skyblock/auctions_ended" || path === "skyblock/bazaar") {
            noRateLimit = true;
            includeApiKey = false;
        }
